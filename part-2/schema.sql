...
comment on table orders is 'purchase events';
comment on column orders.id is 'ID';
comment on column orders.shopper is 'shopper who made the purchase';
...
comment on column sales.order is 'purchase event';
comment on column sales.product is 'product of which 1 unit was included';

create function section_products(
  section integer, out product text, out section text
) returns setof record language sql stable as $$
  select products.name, sections.name from products, sections
...
create function shopper_orders(
  shopper integer, out order integer, out cost integer
) returns setof record language sql stable as $$
  select orders.id, to_char(sum(products.price) / 100.0, 'FM0000.99')
  from orders, sales, products
  where orders.shopper = shopper
  and sales.order = orders.id
  and products.id = sales.product
  order by orders.id;
$$;

comment on function shopper_orders(integer) is 'In: ID of a shopper. Out: ID and total cost of each of the shopper’s orders.';

create function real_shoppers(
  out shopper text, out order_count integer
) returns setof record language sql stable as $$
  select shoppers.name, count(orders.id) from shoppers, orders
  where orders.shopper = shoppers.id
  order by shoppers.name;
$$;

comment on function real_shoppers() is 'Out: name of each shopper with 1 or more orders and the count of those orders.';
